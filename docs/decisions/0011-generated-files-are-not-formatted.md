...
directory exclusion anywhere above the file decides the path before the built-in
list is consulted.
...
4. For the two path-based entry points (directory discovery and an explicitly
   ...
   recipe, `!package-lock.json` at the repository root. Where any `.primignore`
   in the stack excludes a directory holding the file — the exclusion beside the
   negation, or in a file above it — the override does not apply: gitignore's
   rule that a `!` rule cannot re-include a path under an excluded directory
   decides the path before the built-in list is consulted (#114, FR-4.4).
   `--no-primignore` disables the built-in list along with the rest of the
   `.primignore` stack. There is no separate flag. `--stdin-filepath` and the
   LSP formatting request never consult `.primignore` at all, so neither escape
   applies to them: on those two paths, `generated_by` is unconditional, and
   there is no way to make prim format a listed file over stdin or through the
   LSP. The `.primignore` files consulted for a path are bounded by the
   repository containing whatever prim was pointed at, as FR-4.4b specifies and
   AD-0009 explains, so a `.primignore` outside the repository (for example one
   left in `$HOME`) cannot silently disable the built-in list for every
...
- For a path reached by directory discovery or named explicitly, anyone who
  ...
  code change: a `!name` line in a committed `.primignore`, so long as no
  `.primignore` in the stack excludes a directory holding the file, or
  `--no-primignore` for a one-off run. Neither works over `--stdin-filepath` or
  the LSP: those two paths never consult `.primignore`, so the decline there is
  unconditional (Decision item 4).
