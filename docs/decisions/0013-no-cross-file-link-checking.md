...
`SELECTABLE_RULES`, leaving a floor tier of 12 defect rules. Amends AD-0012's
floor tier list.
...
1. **MD057 is removed from `SELECTABLE_RULES`.** It is in neither tier, and
   neither `prim_mdlint_disable` nor `prim_mdlint_enable` can bring back a rule
   prim never runs. `prim_mdlint_enable = MD057` is refused as a withheld id,
   the same answer MD072 and MD082 get: the argument above is that prim should
   not answer the cross-file link question at this layer at all, not that it
   should answer it on request.
...
3. **The fixture-coverage test becomes exact.** Every `SELECTABLE_RULES` entry
   has exactly one fixture in `rule_fixtures.rs`, with no exception. MD057 joins
   MD082 in `withheld_rules_never_run_at_any_tier_or_enable`, which asserts a
   rule runs at neither tier and cannot be added by `prim_mdlint_enable` — the
   same shape already used for a rule prim decided not to run. It stays out of
   `tests/withheld_rules.rs`, which is reserved for rules whose refusal rests on
   a property of the dependency — a rumdl bump could make one of those start
   reporting and invalidate the reason recorded for it. MD057's refusal rests on
   this decision instead: prim could make the rule fire tomorrow by passing
   `source_file` through, and declines to.
...
- **The floor tier is 12 defect rules, not 13.** `docs/SPEC.md`, `docs/USAGE.md`
  and AD-0012 are updated, and MD057 is listed among the withheld rules —
  neither tier runs it and `prim_mdlint_enable` refuses it.
